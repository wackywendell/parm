...
inline neighborlist* neighborlistL(Box *box, const double innerradius, const double outerradius){
...
inline ContactTracker* ContactTrackerD(Box *box, atomgroup *atoms, vector<double> dists){
...
struct WallAtom : atomref {
    public:
        ...
        flt epsilon;
    public:
        WallAtom(atom *a, flt sigma, flt epsilon=1.0) : 
...
class WalledBox2D : public OriginBox {
    ...
    public:
        ...
        flt resizeV(flt newV){
            flt curV = V();
            return resize(powflt(newV/curV, OVERNDIM));
        }
