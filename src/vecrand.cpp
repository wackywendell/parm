...
#ifdef VEC2D
Vec rand_vec() { return Vec(gauss(), gauss()); }

Vec rand_vec_boxed() { return Vec(uniformrand(), uniformrand()); }
#else
Vec rand_vec() { return Vec(gauss(), gauss(), gauss()); }
...
BivariateGauss::BivariateGauss(const flt s1, const flt s2, const flt corr)
    : distro(0, 1), gauss(randengine, distro) {
    set(s1, s2, corr);
};
...
Matrix best_rotation_matrix(Eigen::Matrix<flt, Eigen::Dynamic, NDIM> &from,
    ...
    Eigen::JacobiSVD<Matrix> svd(from.adjoint() * to,
