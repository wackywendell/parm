...
// on MAC OS, g++ is short for clang, and they already define the following
...
#else
/**
The basic floating point type used in the simulations. The entire package can be
compiled with
...
typedef std::complex<flt>
    cmplx;  // need the std:: for SWIG complex.i, not sure why
...
/**
A constant equal to \f$\frac{1}{d}\f$, where \f$d\f$ is the number of
dimensions.
*/
...
inline Vec2 rotate_inv(Vec2 v, uint i) { return rotate(v, 4 - i); };
...
/** Generate a random number between 0 and 1, using the "global" random number
 * generator.
*/
flt rand01();
/** Generate a random vector from a Gaussian distribution, i.e.\
\f$P(x)=\frac{1}{\sqrt{2 \pi}} e^{\frac{-x^2}{2\pi}}\f$, and similarly for
\f$y\f$ and \f$z\f$.

In terms of spherical coordinates, directionality is uniform on a sphere, and
the radial
...
/**
A class for generating two random numbers from a Gaussian distribution, with a
given correlation.
...
long double to_LD(
    double e); /**< Go to and from Long Doubles. Useful from Python.*/
double from_LD(
    long double e); /**< Go to and from Long Doubles. Useful from Python.*/
vector<long double> LDVector(vector<double> dists); /**< Go to and from Long
                                                       Doubles. Useful from
                                                       Python.*/
