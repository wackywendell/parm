...
Collection::Collection(sptr<Box> box, sptr<AtomGroup> atoms,
                       vector<sptr<Interaction> > is,
                       vector<sptr<StateTracker> > ts,
                       vector<sptr<Constraint> > cs, bool should_initialize)
...
CollectionSol::CollectionSol(sptr<Box> box, sptr<AtomGroup> atoms,
    ...
    : Collection::Collection(box, atoms, interactions, trackers, constraints,
                             false),
      dt(dt0),
      damping(damp),
      force_mag(damp),
...
void CollectionSol::set_constants() {
    ...
    if (dampdt > 1e-4)
        ...
        sigmar = sqrt(2 * dampdt / 3 - dampdt * dampdt / 2 +
                      7 * dampdt * dampdt * dampdt / 30);
...
void CollectionSol::timestep() {
    ...
    for (uint i = 0; i < m.size(); i++) {
        ...
        VecPair vecpair;
        if (damping > 0)
            vecpair = gauss.gen_vecs();
        else
            vecpair.setZero();
        // vecpair[0] is drG, and vecpair[1] is dvG
        m[i].x +=
            m[i].v * (c1 * dt) + m[i].a * (c2 * dt * dt) + vecpair.col(0) * r0;
...
CollectionSolHT::CollectionSolHT(sptr<Box> box, sptr<AtomGroup> atoms,
    ...
    : Collection(box, atoms, interactions, trackers, constraints, false),
      ...
      damping(damp),
      desT(T),
      gauss(sqrt(2.0 * desT * damping / dt)) {
    set_gauss();
    initialize();
};
...
void CollectionNLCG::timestep() {
    ...
    // August 4, 1994
    // An electronic copy of this article is available by anonymous FTP to
    // WARP.CS.CMU.EDU (IP address 128.2.209.103) under the filename
    // quake-papers/painless-conjugate-gradient.ps.

    // Uses a combination of
    // B4. Nonlinear Conjugate Gradients with Newton-Raphson and Fletcher-Reeves
    // B5. Preconditioned Nonlinear Conjugate Gradients with Secant and
    // Polak-Ribi`ere
    ...
    for (sec = 0; sec < secmax; sec++) {
        ...
        // if(alphamax > 0 and alphafac > 0 and alphafac < alphamin) alphafac =
        // alphamin;
        if (alphamax > 0 and alphafac < -alphamax) alphafac = -alphamax;
        // if(alphamax > 0 and alphafac < 0 and alphafac > -alphamin) alphafac =
        // -alphamin;
        ...
        if ((sec > 1) and (afrac > 0) and (abs(alpha) < abs(dxsum)) and
            (abs(alpha) / abs(dxsum) < afrac))
            break;
...
CollectionNLCGV::CollectionNLCGV(sptr<Box> box, sptr<AtomGroup> atoms,
                                 ...
                                 vector<sptr<StateTracker> > trackers,
                                 vector<sptr<Constraint> > constraints,
                                 const flt kmax, const uint secmax,
                                 const flt seceps)
    : Collection(box, atoms, interactions, trackers, constraints),
      dt(dt),
      seceps(seceps),
...
void CollectionNoseHoover::timestep() {
    ...
    //~ flt oldxi = xi;
    xi = solve_cubic(4 / dt - z0, 4 / dt / dt - 4 * z0 / dt,
                     -(z0 * 4 / dt / dt) - z1, xi);
...
flt CollectionNoseHoover::hamiltonian() {
    flt H = (kinetic_energy() + potential_energy() + (xi * xi * Q / 2) +
             (degrees_of_freedom() * lns * T));
    assert(not isnan(H));
    return H;
}
...
void CollectionGaussianT::set_forces(bool constraints_and_a,
                                     bool shouldwesetxi) {
...
void CollectionGear4A::timestep() {
    ...
    for (uint i = 0; i < g.size(); i++) {
        ...
        g[i].x +=
            g[i].v * dt + g[i].a * (dt * dt / 2) + bs[n] * (dt * dt * dt / 6);
...
void CollectionRK4::timestep() {
    ...
    for (uint i = 0; i < g.size(); i++) {
        ...
        adat.Kxd = a->v * dt;
        adat.Kvd = a->a * dt;
        a->x += (adat.Kxa + adat.Kxd) / 6 + (adat.Kxb) / 3 -
                (adat.Kxc * (2.0 / 3.0));
        a->v += (adat.Kva + adat.Kvd) / 6 + (adat.Kvb) / 3 -
                (adat.Kvc * (2.0 / 3.0));
    }
    update_constraint_positions();
    update_constraint_velocities();
...
void CollectionGear4NPH::timestep() {
    ...
    for (uint m = 0; m < ncorrec; m++) {
        ...
        /// Correction
        flt Vfac =
            (-(2.0 / NDIM / NDIM) * (dV / V) * (dV / V)) + (ddV / V / NDIM);
        flt newddV = (newP - P) / Q;
...
vector<sptr<Interaction> > CollectionGear4NPT::tointerpair(
    vector<sptr<InteractionPairsX> > &v) {
...
void CollectionGear4NPT::set_forces(bool seta) {
    ...
    vector<sptr<Interaction> >::iterator it;
    for (it = interactions.begin(); it < interactions.end(); ++it) {
        sptr<InteractionPairsX> inter =
            boost::dynamic_pointer_cast<InteractionPairsX>(*it);
...
void CollectionVerletNPT::timestep() {
    ...
    if (QP > 0) {
        ...
        flt Vfac1 = pow(newV / V, OVERNDIM),
            Vfac2 = pow(2 * newV / (newV + V), OVERNDIM);
        for (uint i = 0; i < g.size(); i++) {
...
Event CollectionCDgrid::next_event(AtomID a) {
    ...
    for (Grid::pair_iter j = grid.pairs(a); j != j.end(); ++j) {
        ...
        if (make_event(*box, e2, *j, a, sigma, curt) and e2 < e) {
            e = e2;
        };
    }
    assert(e.t >= curt);
...
void CollectionCD::reset_events() {
    ...
    for (uint i = 1; i < atoms->size(); i++) {
        for (uint j = 0; j < i; j++) {
            ...
            //~ std::cerr << i << "-" << j;
            Event e;
            if (make_event(*box, e, atoms->get_id(j), atoms->get_id(i), sigma,
                           curt)) {
                events.insert(e);
                //~ std::cerr << " MADE!";
            };
...
bool CollectionCD::take_step(flt tlim) {
    ...
    for (uint i = 0; i < atoms->size(); i++) {
        ...
        for (ait = badatoms.begin(); ait != badatoms.end(); ++ait) {
            ...
            //~ cerr << "sigma " << sigma << "\n";
            Event testevent;
            if (make_event(*box, testevent, *ait, atoms->get_id(i), sigma,
                           curt)) {
...
#define tol 1e-3
        if (d1 > tol and d2 > tol and d3 > tol) {
            ...
            printf("%.4f (%.4f), %.4f (%.4f), %.4f (%.4f) : %.4f\n", (double)x1,
                   (double)d1, (double)x2, (double)d2, (double)x3, (double)d3,
                   (double)x);
        }
        return x;
    }
...
flt solve_cubic_fast(flt b, flt c, flt d) {
    // from Wikipedia
    flt determ = (pow(2 * pow(b, 2) - 9 * b * c + 27 * d, 2) -
                  4 * pow(b * b - 3 * c, 3));
    if (determ < 0) printf("bad determ: %.4f\n", (double)determ);
