...
parser.add_option('--hphobscale', default='monera')
parser.add_option('--ph3', action='store_true')
parser.add_option('--hphobmethod', type='choice',default='geometric',
                    choices=('geometric', 'geometriczero', 'arithmetic',
                                'arithmeticzero'))
...
if not hasattr(simpdb, opts.hphobscale):
    print("Hydrophobicity scale %s not found" % opts.hphobscale, file=sys.stderr)
    exit(1)

hydroindex = dict(getattr(simpdb, opts.hphobscale))
Hs = [hydroindex[k] for k in sorted(simpdb.res31)]
...
hphob, HPneighbors = (
    simpdb.make_hydrophobicity(avecs, LJe, #Hphobs, #instead of LJe
                        ...
                        method=opts.hphobmethod) 
    if LJe > 0
    else (None,None))
    
print("Hydrophobicity made with scale %r and method %r: %.2f - %.2f" % 
            (opts.hphobscale, opts.hphobmethod, min(hydroindex.values()), max(hydroindex.values())))
