...
conversion = 0.0017186 # 1 cal/mol/avogadro / (293 K boltzmann)
# ACENOVH
# 0123456
...
fname = mydir + ('blengths/eisenberg-mclachlan-coeffs.tsv')
with open(fname) as f:
    atom_sigmas = [linetotypes(l, str, float) for l in f if l.strip()]
name_to_sigma = dict([(n, sig*conversion) for n,sig in atom_sigmas])

alice_sizes, hphob_sigmas = defaultdict(dict), defaultdict(dict)
with open(mydir + 'blengths/alicesizes-AMBERnames.tsv') as f:
    for l in f:
        if not l.strip('\n'): continue
        if l.strip(' \t')[0] == '#': continue
        r,aname,s,ambername = linetotypes(l, str, str, float, str)
        if ambername is '': continue
        alice_sizes[r][aname] = s
        try:
            hphob_sigmas[r][aname] = name_to_sigma[ambername]
        except ValueError:
            print("name", ambername, file=sys.stderr)
            raise
...
def getsizelocs(res, opts):
    ...
    for a in locs:
        ...
        atomsize = alice_sizes[res][a] if a in alice_sizes[res] else alice_sizes['X'][a]
        atomH = hphob_sigmas[res][a] if a in hphob_sigmas[res] else hphob_sigmas['X'][a]
        atommass = masses[a[0]]
        sizes[a] = [atommass, atomsize, atomH]
