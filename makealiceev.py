...
enegs=dict(C=2.55, N=3.04, O=3.44, H=2.20, S=2.58)
...
def getsizelocs(res, bonds, opts):
    locs = {}
    ...
    for (r1,a1,r2,a2,x0,k) in bonds:
        bondDict[(r1,a1)].append(enegs[a2[0]])
        bondDict[(r2,a2)].append(enegs[a1[0]])
    ...
    for a in locs:
        ...
        atommass = masses[a[0]]
        eneglist = bondDict[(0, a)]
        
        #~ enegdiff = max(eneglist) - min(eneglist)
        #~ if len(eneglist) == 1: enegdiff = abs(eneglist[0] - enegs[a[0]])
        #~ Hphob = (2.0 - enegdiff) / 2.0
        
        enegdiffs = [abs(e - enegs[a[0]]) for e in bondDict[(0, a)]]
        Hphob = (1.5 - max(enegdiffs)) / 2.0
        
        print(res, a, eneglist, [float('%.2f' % e) for e in enegdiffs], '%.2f' % Hphob)
        
        sizes[a] = [atommass, atomsize, Hphob]
...
if __name__ == "__main__":
    ...
    sizelocs = getsizelocs(res, bonds, opts=opts)
